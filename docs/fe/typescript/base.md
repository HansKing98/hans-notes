...
```ts
let sym: symbol = Symbol('hansking')
sym = Symbol('网友茂茂') // OK
sym = '网友茂茂' // Error
```
...
// 任意类型
const anyArray: any[] = ['hansking', 18, {}]
```
...
// 任意类型
const anyArray: Array<any> = ['hansking', 18, {}]
```
...
```ts
const tuple: [string, number] = ['hansking', 18]
...
```ts
type Name = 'hansking' | 'hansking98' | '网友茂茂'
```
